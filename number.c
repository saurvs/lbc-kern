     ...
     int length, scale;
{
  ...
  temp = (bc_num) kmem_alloc (sizeof(bc_struct), KM_SLEEP);
  temp->n_sign = PLUS;
  temp->n_len = length;
  temp->n_scale = scale;
  temp->n_refs = 1;
  temp->n_ptr = (char *) kmem_alloc (length+scale, KM_SLEEP);
    ...
    bc_num *num;
{
  ...
  if ((*num)->n_refs == 0) {
    if ((*num)->n_ptr)
      kmem_free ((*num)->n_ptr, (*num)->n_len + (*num)->n_scale);
    kmem_free (*num, sizeof(bc_struct));
  }
  *num = NULL;
}
     ...
     int scale;
{
  ...
  if (scale1 < scale)
    ...
    extra = 0;
  num1 = (unsigned char *) kmem_alloc (n1->n_len+n1->n_scale+extra+2, KM_SLEEP);
  memset (num1, 0, n1->n_len+n1->n_scale+extra+2);
  memcpy (num1+1, n1->n_value, n1->n_len+n1->n_scale);

  len2 = n2->n_len + scale2;
  num2 = (unsigned char *) kmem_alloc (len2+1, KM_SLEEP);
  ...
  *(num2+len2) = 0;
  ...
  mval = (unsigned char *) kmem_alloc (len2+1, KM_SLEEP);
  ...
  *quot = qval;
  ...
  kmem_free (mval, len2+1);
  kmem_free (num1, n1->n_len+n1->n_scale+extra+2);
  kmem_free (num2, len2+1);
      ...
      int *str_mlen;
{
  ...
  if (num->n_scale > 0)
    *str_mlen = num->n_len + num->n_scale + 2 + signch;
  else
    *str_mlen = num->n_len + 1 + signch;
  
  str = (char *) kmem_alloc (*str_mlen, KM_SLEEP);
