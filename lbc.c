...
static bc_num Bget(lua_State *L, int i)
{
 ...
 switch (lua_type(L,i))
 {
  ...
  default:
   return *((void**)luaL_checkudata(L,i,MYNAME));
 }
 return NULL;
}
...
static int Btrunc(lua_State *L)			/** trunc(x,[n]) */
{
 bc_num a=Bget(L,1);
 bc_num c=NULL;
 bc_divide(a,bc_one,&c,luaL_optinteger(L,2,0));
 Bnew(L,c);
 return 1;
}
...
static const luaL_Reg bc_methods[] = {
	{ "add",	Badd	},
	{ "compare",	Bcompare},
	{ "digits",	Bdigits	},
	{ "div",	Bdiv	},
...
static int
luaopen_lbc(lua_State *L)
{
 bc_init_numbers();
 luaL_newmetatable(L,"lbc");
 lua_pushvalue(L, -1);
 ...
 lua_pushvalue(L,-2);
 lua_settable(L,-3);*/
 luaL_newlib(L, bc_methods);
// lua_createtable(L, 1, 0);
// luaL_setfuncs(L, bc_methods, 0);
 return 1;
}

MODULE(MODULE_CLASS_MISC, lbc, "lua");

static int
lbc_modcmd(modcmd_t cmd, void *data)
{
    int error;

    switch (cmd) {
	case MODULE_CMD_INIT:
	    error = klua_mod_register("lbc", luaopen_lbc);
	    break;
	case MODULE_CMD_FINI:
	    error = klua_mod_unregister("lbc");
	    break;
	default:
	    error = ENOTTY;
	    break;
